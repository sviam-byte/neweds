"""Sliding window analysis and 3D cube scanning extracted from engine.py."""
...
def analyze_sliding_windows(
    ...
    *,
    ...
    def _compute_one(start: int) -> tuple[int, int, float, Optional[np.ndarray]]:
        end = start + w
        if end > n:
        ...
        try:
            ...
            return int(start), int(end), float(score) if np.isfinite(score) else float("nan"), (mat if return_matrices else None)
