"""Pair resolution logic for connectivity computation.

Handles: parsing user-supplied pair text, building neighbor pairs from
coordinates, auto-selecting pair mode based on matrix size, and OOM guards.
"""
...
def build_neighbor_pairs(
    coords_df: Optional[pd.DataFrame],
    columns: pd.Index,
    kind: str = "26",
    radius: int = 1,
) -> list[tuple[int, int]]:
...
def resolve_pairs(
    ...
    auto_thr: int = 500,
    **kwargs,
) -> tuple[Optional[list[tuple[int, int]]], str, dict]:
    ...
    if pairs_idx is None:
        ...
        if mat_gb > 8.0:
            logging.error("[memory] Matrix %dx%d=%.1fGB>8GB, forcing random.", n_cols, n_cols, mat_gb)
            rng = np.random.default_rng(12345)
    ...
    if pairs_idx is not None:
        ...
        meta["pairs_explain"] = (
            "Матрица считается упрощённо: только по выбранным парам."
        )
