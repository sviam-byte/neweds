"""Оркестратор batch-анализа: общий код для CLI и сценариев пакетной обработки."""
...
def iter_supported_input_files(folder: str, recursive: bool = False) -> list[str]:
    """Возвращает список поддерживаемых файлов из каталога."""
...
def safe_slug(text: str) -> str:
    """Превращает произвольную строку в безопасное имя каталога."""
...
def write_batch_manifest(rows: list[dict[str, Any]], manifest_csv: str) -> None:
    """Пишет CSV-манифест batch-прогона со стабильным набором колонок."""
...
def make_batch_zip(out_root: str) -> str:
    """Упаковывает весь каталог результатов batch-прогона в ZIP."""
