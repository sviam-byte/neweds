...
class HTMLReportGenerator:
    ...
    def _render_autocorr_section(self) -> str:
        ...
        if isinstance(ex, dict) and ex:
            ...
            for col, item in list(ex.items())[:3]:
                ...
                img_ts = self._b64_png(
                    plots.plot_timeseries_before_after(before_ts, after_ts, title)
                )
                img_acf = self._b64_png(
                    plots.plot_acf_before_after(before_ts, after_ts, f"ACF: {col}")
                )
    ...
    def _build_scans_interactive_html(self, payload: dict) -> str:
        ...
        return (
            "".join(blocks)
            + f"<script type='application/json' id='scan_data_{prefix}'>{js}</script>"
        )
    ...
    def generate(self, output_path: str, **kwargs) -> str:
        ...
        try:
                ...
                if include_series_files:
                    ...
                    try:
                        if series_xlsx.exists():
                            links.append(
                                f"<a href='{html.escape(series_xlsx.name)}' download>{html.escape(series_xlsx.name)}</a>"
                            )
                        if series_csv.exists():
                            links.append(
                                f"<a href='{html.escape(series_csv.name)}' download>{html.escape(series_csv.name)}</a>"
                            )
                    except Exception:
                        links = []
                    ...
                    if links:
                        series_files_html = (
                            "<div class='meta'><b>Ряды (файлы):</b> " + " • ".join(links) + "</div>"
                        )
            except Exception:
                series_files_html = ""

            b64_raw = self._b64_png(
                plots.plot_timeseries_panel(
                    raw_df, "Ряды: RAW (общий масштаб Y)", y_domain=y_domain
                )
            )
            b64_proc = self._b64_png(
                plots.plot_timeseries_panel(
                    proc_df,
                    "Ряды: после предобработки/auto-diff (общий масштаб Y)",
                    y_domain=y_domain,
                )
            )
            ...
            if p0.get("dropped_columns"):
                prep_lines.append(
                    "<b>Удалено</b>: "
                    + html.escape(", ".join(map(str, p0.get("dropped_columns", []))))
                )
            if p0.get("steps_global"):
                prep_lines.append(
                    "<b>Шаги</b>: "
                    + html.escape(" | ".join(map(str, p0.get("steps_global", [])[:12])))
                )
            ad = prep.get("autodiff") or {}
            if ad.get("enabled"):
                prep_lines.append(
                    "<b>Auto-diff</b>: дифференцированы "
                    + html.escape(", ".join(map(str, ad.get("differenced", []))) or "—")
                )
            ...
            if dr.get("enabled"):
                ...
                try:
                    ev_txt = (
                        f", explained_var={float(explained):.4g}" if explained is not None else ""
                    )
                except Exception:
                    ev_txt = ""
                prep_lines.append(
            ...
            sections.insert(
                ...
                f"{prep_html}"
                f"{series_details}"
                f"{table_details}" + "".join(harm_cards[:8]) + "</section>",
            )
        except Exception:
            pass
            ...
            if not diag:
                ...
                for name, d in diag.items():
                    ...
                    if include_fft_plots:
                        try:
                            if (
                                getattr(self.tool, "data", None) is not None
                                and name in self.tool.data.columns
                            ):
                                fft_b64 = self._plot_fft_b64(
                                    self.tool.data[name],
                                    f"FFT spectrum: {name}",
                                    fs=float(getattr(self.tool, "fs", 1.0)),
                                )
                                fft_html = (
                    ...
                    cards.append(
                        ...
                        "<div><b>Permutation H</b>: "
                        + _fmt(d.get("permutation_entropy"))
                        + "</div>"
        ...
        for k, variant in enumerate(variants, start=1):
            ...
            if meta.get("partial"):
                ...
                if p.get("pairwise_policy") == "others":
                    meta_lines.append(
                        "Partial: для пары (Xi,Xj) исключено линейное влияние всех остальных переменных."
                    )
                elif p.get("pairwise_policy") == "custom":
                    cc = p.get("custom_controls") or []
                    meta_lines.append(
                        "Partial: исключено влияние control="
                        + html.escape(", ".join(map(str, cc)))
                        + "."
                    )
                else:
            ...
            if isinstance(cube, dict) and (cube.get("points") or []):
                try:
                    b64 = self._plot_cube3d_b64(
                        cube.get("points") or [], f"{variant}: размер окна×лаг×положение"
                    )
                    cube_html = (
                        "<div class='card'><h3>3D: размер окна × лаг × положение</h3><img src='data:image/png;base64,"
                        + b64
                        + "'/></div>"
                    )
                except Exception:
            ...
            try:
                ...
                if dfp is not None and not getattr(dfp, "empty", True):
                    if is_pvalue_method(variant):
                        ...
                        view = (
                            dfp.assign(absval=dfp["value"].abs())
                            .sort_values("absval", ascending=False)
                            .head(20)
                            .drop(columns=["absval"])
                        )
                    rows = [
                        "<table class='pairs'><thead><tr><th>src</th><th>tgt</th><th>value</th><th>flag</th></tr></thead><tbody>"
                    ]
                    for _, r in view.iterrows():
                        rows.append(
                            f"<tr><td>{html.escape(str(r['src']))}</td><td>{html.escape(str(r['tgt']))}</td><td>{float(r['value']):.4g}</td><td>{html.escape(str(r.get('flag', '')))}</td></tr>"
                        )
                    rows.append("</tbody></table>")
                    pair_table_html = (
                        "<div class='card'><h3>Сводка по парам (топ)</h3>"
                        + "".join(rows)
                        + "</div>"
                    )
            except Exception:
                ...
                try:
                    ...
                    if isinstance(sc, dict):
                        payload = self._build_scan_payload(
                            variant=variant, anchor=anchor, cols=cols, sc=sc
                        )
