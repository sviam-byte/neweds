...
class ExcelReportWriter:
    ...
    def write(self, save_path: str, **kwargs) -> str:
        ...
        disable_images = str(os.getenv("TS_TOOL_DISABLE_EXCEL_IMAGES", "0")).strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
        ...
        max_image_n = int(
            kwargs.get("excel_max_image_n", os.getenv("TS_TOOL_EXCEL_MAX_IMAGE_N", 300) or 300)
        )
        max_matrix_sheet_n = int(
            kwargs.get(
                "excel_max_matrix_sheet_n", os.getenv("TS_TOOL_EXCEL_MAX_MATRIX_N", 5000) or 5000
            )
        )
        ...
        for variant, mat in (getattr(self.tool, "results", {}) or {}).items():
            ...
            if arr.ndim != 2 or arr.shape[0] != arr.shape[1]:
                ...
                warnings.append(
                    f"{variant}: матрица не квадратная или некорректная, лист записан без графиков."
                )
                continue
            ...
            if n <= max_matrix_sheet_n:
                ...
                ws.append(
                    [
                        "warning",
                        f"matrix_too_large_for_excel_sheet: n={n}, limit={max_matrix_sheet_n}",
                    ]
                )
            ...
            try:
                ...
                if isinstance(ar_df, pd.DataFrame) and not ar_df.empty:
                    ...
                    for idx, col in enumerate(ar_df.columns, start=1):
                        ...
                        ws_ar.column_dimensions[get_column_letter(idx)].width = min(
                            max(12, max(sample) + 2), 28
                        )
