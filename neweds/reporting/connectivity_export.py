...
def save_nodes_csv(
    data_dir: str, names: list[str], coords: dict[str, tuple[Any, Any, Any]] | None = None
) -> str:
...
def export_connectivity_matrix(
    ...
    if n <= int(policy.dense_n_limit):
        ...
        if bool(getattr(policy, "dense_csv", True)):
            ...
            pd.DataFrame(arr, index=names, columns=names).to_csv(
                os.path.join(data_dir, dense_csv_file), index=True
            )
