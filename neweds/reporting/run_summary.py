...
def build_run_summary_ru(tool, *, run_dir: str | None = None) -> str:
    ...
    if isinstance(sb, dict):
        ...
        lines.append(
            f"- бинов (активных): {sb.get('active_bins', '?')} из {sb.get('total_grid_bins', sb.get('total_bins', '?'))}"
        )
        ...
        try:
            ...
            if isinstance(ac, dict):
                ...
                if bmed is not None and amed is not None:
                    try:
                        lines.append(
                            f"- lag=1 corr (медиана): до={float(bmed):.3g}, после={float(amed):.3g}"
                        )
                    except Exception:
                        pass
                if red is not None:
    ...
    try:
        qc_enabled = bool(
            (getattr(tool, "results_meta", {}) or {}).get("__run__", {}).get("qc_enabled", True)
        )
    except Exception:
        qc_enabled = True
