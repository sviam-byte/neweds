...
def analyze_graph_topology(
    ...
    if len(node_names) != n:
        node_names = [str(name) for name in node_names[:n]] + [
            f"node_{i}" for i in range(len(node_names), n)
        ]
    ...
    degree = (
        dict(graph.in_degree(weight="weight")) if directed else dict(graph.degree(weight="weight"))
    )
