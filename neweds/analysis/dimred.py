...
def _variance_select(data: pd.DataFrame, target_n: int) -> DimRedResult:
    ...
    return DimRedResult(
        reduced=reduced, mapping=mapping, meta={"method": "variance", "k": len(keep)}
    )
...
def _random_select(data: pd.DataFrame, target_n: int, seed: int) -> DimRedResult:
    ...
    return DimRedResult(
        reduced=reduced,
        mapping=mapping,
        meta={"method": "random", "k": len(keep), "seed": int(seed)},
    )


def _spatial_bin(
    data: pd.DataFrame, coords_df: pd.DataFrame | None, target_n: int, bin_size: int
) -> DimRedResult:
    ...
    cdf["__bin_x"] = np.floor(
        pd.to_numeric(cdf[xcol], errors="coerce") / max(1, int(bin_size))
    ).astype("Int64")
    cdf["__bin_y"] = np.floor(
        pd.to_numeric(cdf[ycol], errors="coerce") / max(1, int(bin_size))
    ).astype("Int64")
    cdf["__bin_z"] = np.floor(
        pd.to_numeric(cdf[zcol], errors="coerce") / max(1, int(bin_size))
    ).astype("Int64")
...
def _pca_reduce(
    ...
    *,
    ...
    if solver_norm == "gram" or (
        (priority or "auto").strip().lower()
        in {"explained_variance", "variance", "evr", "explained"}
...
def _kmeans_reduce(data: pd.DataFrame, target_n: int, seed: int, batch_size: int) -> DimRedResult:
    ...
    if k >= n:
        mapping = pd.DataFrame(
            {"source": list(data.columns), "target": list(data.columns), "weight": 1.0}
        )
        return DimRedResult(
            reduced=data.copy(),
            mapping=mapping,
            meta={"method": "kmeans", "k": n, "batch_size": int(batch_size)},
        )

    km = MiniBatchKMeans(
        n_clusters=k, batch_size=int(max(32, batch_size)), random_state=int(seed), n_init=3
    )
