        ...
        for src in range(n):
            for tgt in range(n):
                ...
                if weight is None or np.isnan(weight):
                    continue
                if (invert_threshold and weight < threshold) or (
                    not invert_threshold and abs(weight) > threshold
                ):
