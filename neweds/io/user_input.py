...
class RunSpec:
    ...
    window_sizes_grid: list[int] | None
    window_min: int
    window_max: int
    window_step: int
    window_size: int
    ...
    def explain(self) -> str:
        ...
        if self.window_sizes:
            ...
            lines.append(
                f"main_windows={self.window_sizes} stride={stride} policy={self.window_policy}"
            )
        else:
        ...
        lines.append(
            f"preprocess={'on' if self.preprocess else 'off'} options={self.preprocess_options or {}}"
        )
        lines.append(
            f"main_window_cube={self.window_cube_level} (eval_limit={self.window_cube_eval_limit})"
        )
        ...
        if self.include_scans:
            ...
            if self.window_sizes_grid:
                ...
                lines.append(
                    f"window_grid=min:{self.window_min} max:{self.window_max} step:{self.window_step}"
                )
            lines.append(
...
def build_run_spec(user_cfg: dict[str, Any], *, default_max_lag: int = 12) -> RunSpec:
    ...
    window_stride = (
        int(window_stride_raw)
        if window_stride_raw is not None and str(window_stride_raw).strip()
        else None
    )
    ...
    window_cube_level = (
        str(user_cfg.get("window_cube", user_cfg.get("window_cube_level", "off"))).strip().lower()
    )
    ...
    window_size = _parse_int(
        user_cfg.get("window_size", (window_min + window_max) // 2),
        (window_min + window_max) // 2,
        min_v=2,
    )
    ...
    cube_matrix_limit = _parse_int(
        user_cfg.get("cube_matrix_limit", cube_eval_limit), cube_eval_limit, min_v=1, max_v=10000000
    )

    cube_gallery_mode = (
        str(user_cfg.get("cube_gallery_mode", "extremes") or "extremes").strip().lower()
    )
