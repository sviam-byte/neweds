...
### Добавлено
- Central metric registry with decorator support (`@register_metric(...)`) и `@dataclass(frozen=True, slots=True)` `Metric`. Все метрики снабжены категорией (`correlation` / `information` / `spectral` / `ordinal` / `causal`) и описанием.
...
- `neweds/methods.py` — compatibility facade для каталога методов (`STABLE_METHODS`, `EXPERIMENTAL_METHODS`, `DIRECTED_METHODS`, `PVAL_METHODS`, `METHOD_INFO`); source of truth теперь `neweds.metrics.registry`.
...
- Тест-сьют расширен ground-truth, snapshot, CLI и architecture-регрессиями; актуальный статус прохождения подтверждается CI.
