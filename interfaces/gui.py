...
class App(tk.Tk):
    ...
    def __init__(self) -> None:
        ...
        # Словарь чекбоксов методов анализа.
        # methods_vars — основной ключ (как в legacy-UI), method_vars — алиас для совместимости.
        self.methods_vars: dict[str, tk.BooleanVar] = {}
        self.method_vars = self.methods_vars
    ...
    def _build_settings_panel(self) -> None:
        ...
        # ===================== МЕТОДЫ =====================
        methods_frame = ttk.LabelFrame(frame, text="Методы анализа")
        methods_frame.pack(fill="x", padx=6, pady=4)

        # Сбрасываем словарь перед созданием чекбоксов, чтобы не копить stale-ссылки.
        self.methods_vars = {}
        self.method_vars = self.methods_vars
        ...
        for i, m in enumerate(all_methods):
            var = tk.BooleanVar(value=(m in default_methods))
            r, c = divmod(i, cols)
            label = m
    ...
    def _get_selected_methods(self) -> list[str]:
        """Возвращает список выбранных методов анализа из UI-чекбоксов."""
        source = self.methods_vars if getattr(self, "methods_vars", None) is not None else self.method_vars
        return [m for m, var in source.items() if var.get()]
    ...
    def _run_tool(self, df: pd.DataFrame, out_dir: str, name_prefix: str) -> str | None:
        ...
        # Явно фиксируем набор методов в runtime-конфиге,
        # чтобы downstream-движок не запускался с пустым списком после рефакторинга UI.
        runtime_config: dict[str, object] = {"methods": methods}
